...
{
    public class DropdownChooseAnyBigScroll : BaseDropdown
    {
        ...
        {
            for (int i = 0; i < Database.Instance.DatabaseInventory.BigScrollsAll.Length; i++)
            {
                this.dropdownChoiceOptions.Add(Database.Instance.DatabaseInventory.BigScrollsAll[i].Name);
            }
        ...
        private IEnumerator WaitWithOnClickActionTillButtonReleased()
        {
            ...
            debugModeItemsChoiceController.TryToUseBigScroll(dropdown.value);
        }
    }
}

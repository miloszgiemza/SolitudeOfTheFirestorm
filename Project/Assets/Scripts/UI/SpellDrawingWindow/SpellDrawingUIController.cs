...
public class SpellDrawingUIController : MonoBehaviour
{
    ...
    private void Start()
    {
        CreateChoiceButtons(SpellsController.Instance.SpellsAvaliableAtTheStartOfRound, SpellsController.Instance.SetCurrentSpellForThisTurn);
    }
    ...
    public void ShowWindowAndUpdateButtons(Action<int> MethodForButtons, Action MethodToReDrawSpells)
    {
        window.SetActive(true);

        for(int i = 0; i < buttons.Count; i++)
        {
            buttons[i].UpdateButton(SpellsController.Instance.SpellsAvaliableForThisTurn[i].SpellIcon, MethodForButtons);
        }

...
public class EnemyBypassingObstaclesAndAvoidingDangersMovementAllDirections : EnemyNormal
{
    protected new MapPosition[] movementDirections = { new MapPosition(0, 1), new MapPosition(1, 1), new MapPosition(-1, 1), new MapPosition(1, 0), new MapPosition(-1, 0), new MapPosition(0, -1), new MapPosition(1, -1), new MapPosition(-1, -1) };

    public EnemyBypassingObstaclesAndAvoidingDangersMovementAllDirections(Sprite gameplayImage, TooltipParagraph[] descriptionEN, TooltipParagraph[] descriptionPL, 
    ...
    protected List<PositionInGrid> GetOptimalPath(Node[,] nodes)
    {
        ...
        GraphSquaresDiagonalMovementAllDirections graph = new GraphSquaresDiagonalMovementAllDirections(MapPathfinding.Instance.NodesEnemyWalking);
        ...
        for (int i = 0; i < nodes.GetLength(0); i++)
        {
            ...
            SearchAStarHeuristicTileCost search = new SearchAStarHeuristicTileCost(graph, graph.Nodes[tile.Position.X, tile.Position.Y], graph.Nodes[i, 0]);

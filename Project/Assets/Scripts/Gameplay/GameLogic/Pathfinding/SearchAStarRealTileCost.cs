...
{
    public class SearchAStarRealTileCost : BaseSearch
    {
        public SearchAStarRealTileCost(BaseGraph graphValue, Node startNodeValue, Node endNodeValue) : base(graphValue, startNodeValue, endNodeValue) { }
        ...
        public override List<PositionInGrid> SearchAndReturnPath()
        {
            ...
            if(!ReferenceEquals(pathPositions, null) && pathPositions.Count > 0)pathPositions.RemoveAt(0);
        ...
        protected override void ExpandFrontier(Node node)
        {
            for (int i = 0; i < node.Neighbours.Count; i++)
            {
                if (!exploredNodes.Contains(node.Neighbours[i]))
                {
                    ...
                    float newDistanceTravelled = distanceToNeighbour + node.DistanceTravelled + node.MovementCostReal;

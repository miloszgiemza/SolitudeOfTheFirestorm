...
public class StateCastingSpellFromSmallScrollInDebugMode : BasePlayerState
{
    public override PlayerState State => PlayerState.CastingSpellFromSmallScrollInDebugMode;
    ...
    public override void RunUpdate(Player player, GameplayController gameplayController)
    {
        ...
        else if (InputController.Instance.MainInputAssetsWrapper.MobileDevicesMap.MainAction.WasReleasedThisFrame())
        {
            TryCastSpell(InputController.Instance.MainInputAssetsWrapper.MobileDevicesMap.MainActionPosition.ReadValue<Vector2>(),
                ...
                Player.Instance.Attributes[AttributeID.PlayerModifierSpellEffectDuration].CurrentValue, player);
        }
    ...
    {
        //wy³¹cz podœwietlenie
        Player.Instance.SwitchState(PlayerState.IdleDebugMode);
    }

    private void TryCastSpell(Vector2 cursorPos, int modifierDamage, int modifierRange, int modifierEffectLength, Player player)
    {
            ...
            modifierDamage, modifierRange, modifierEffectLength))
        {
            ...
            WrapUpAfterSuccesfullSpell(Map.Instance.MapData, InputController.Instance.MainInputAssetsWrapper.MobileDevicesMap.MainActionPosition.ReadValue<Vector2>(),
                SpellsController.Instance.CurrentSpell, modifierRange, player);
        }
        else
        {
            //PlayerInventoryController.Instance.PlayerInventory.DeselectItemAndNullCurrentItemOnUnsuccesfullUse();
    ...
    private void WrapUpAfterSuccesfullSpell(Tile[,] mapData, Vector2 cursorPos, BaseSpell spell, int modifierRange, Player player)
    {
